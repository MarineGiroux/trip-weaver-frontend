...
const OPTION_LENGTH_ZERO: number = 0;
const ROLE_NAME_CHART_ZERO: number = 0;
const ROLE_NAME_SLICE_ONE: number = 1;
const SELECTED_VALUE_LENGTH_ZERO: number = 0;
const SET_TIMEOUT: number = 0;
...
export class ActionEditComponent {
  ...
  getSelectedLabels(fieldName: string, options: FieldOption[] = []): string[] {
    ...
    if (!selectedValues || !Array.isArray(selectedValues) || selectedValues.length === SELECTED_VALUE_LENGTH_ZERO) {
      return [];
    }
  ...
  formatRoleName(roleName: string): string {
    return roleName.charAt(ROLE_NAME_CHART_ZERO).toUpperCase() + roleName.slice(ROLE_NAME_SLICE_ONE).toLowerCase();
  }

  getSelectedLabel(fieldName: string, options: FieldOption[] = []): string {
    const selectedValue = this.editForm.get(fieldName)?.value;

    if (!selectedValue || !options || options.length === OPTION_LENGTH_ZERO) {
