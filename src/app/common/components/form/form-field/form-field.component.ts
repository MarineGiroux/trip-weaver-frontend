...
const DEFAULT_MIN_LENGTH: number = 2;
const ROWS: number = 2;
...
export class FormFieldComponent implements OnInit {
  ...
  @Input() labelClass: string = 'text-text';
  ...
  ngOnInit(): void {
    if (this.name === 'confirmPassword' && this.parentFormGroup) {
      this.parentFormGroup.statusChanges.pipe(takeUntilDestroyed(this._destroyRef)).subscribe(() => {
